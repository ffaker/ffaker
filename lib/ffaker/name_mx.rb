...
module Faker
  module NameMX
    ...
    # Full name according to gender, possibly with two names, possible with prefix
    def name(gender = :any)
      case gender
        when :any then
          case rand(9)
            when 0, 3, 6, 8 then prefix_female_name
            else prefix_male_name
          end
        when :male then prefix_male_name
        when :female then prefix_female_name
        else raise ArgumentError, "Invalid gender, must be one of :any, :male, :female"
      end
    end
    
    # Full male name possibly with prefix
    def prefix_male_name
      case rand(9)
        when 0, 5 then "#{male_prefix} #{male_name} #{paternal_last_names}"
        else "#{male_name} #{paternal_last_names}"
      end
    end
    
    # Full female name possibly with prefix
    def prefix_female_name
      case rand(9)
        when 0, 5 then "#{female_prefix} #{female_name} #{paternal_last_names}"
        else"#{female_name} #{paternal_last_names}"
      end
    end
    ...
    def male_name
      case rand(9)
        when 0, 5 then "#{first_name(:male)} #{first_name(:male)}"
    ...
    def female_name
      case rand(9)
        when 0, 5 then "#{first_name(:female)} #{first_name(:female)}"
        else first_name(:female)
      end
    end

    # Single name according to gender parameter
