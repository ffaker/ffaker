...
class TestPhoneNumberID < Test::Unit::TestCase
  ...
  def test_phone_number
    ...
    regex = /(#{mobile_prefixes.join('|')}|#{home_prefixes.join('|')})-(\d{7,8})/
  ...
  def test_international_phone_number
    ...
    regex = /\+62(#{mobile_prefixes.join('|')}|#{home_prefixes.join('|')})-(\d{7,8})/
