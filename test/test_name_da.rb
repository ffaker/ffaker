...
class TestFakerNameDA < Minitest::Test
  RU_REGEX = /[А-Я][а-я]+/
  ...
  def test_name
    assert_raises { @tester.name(:hahaha) }
  end
  ...
  def test_first_name
    assert_includes @tester::MALE_FIRST_NAMES, @tester.first_name(:male)
    assert_includes @tester::FEMALE_FIRST_NAMES, @tester.first_name(:female)
    first_names = @tester::MALE_FIRST_NAMES + @tester::FEMALE_FIRST_NAMES
    assert_includes first_names, @tester.first_name(:any)
    assert_includes first_names, @tester.first_name
    assert_raises { @tester.first_name(:hahaha) }
  end
